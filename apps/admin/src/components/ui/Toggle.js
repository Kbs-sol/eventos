...
export function Toggle({ checked, onChange, label, description, disabled, size = 'md' }) {
    ...
    return (_jsxs("label", { className: `flex items-center gap-3 ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} group py-1`, children: [_jsx("button", { type: "button", role: "switch", "aria-checked": checked, disabled: disabled, onClick: (e) => {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        `, children: _jsx("span", { className: `
            ...
            self-center
          ` }) }), label && (_jsx("span", { className: "text-sm font-medium text-gray-700 select-none group-hover:text-gray-900 transition-colors", children: label }))] }));
}
